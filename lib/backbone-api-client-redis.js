...
exports.getMixinBase = function (ParentKlass) {
  ...
  return {
    ...
    _getCollectionHashKey: function () {
      return this._cacheBase + '-hashes-collection';
    },
      ...
      if (method === 'read') {
        ...
        this.requestCache.get({
          ...
          uncachedGet: function forwardUncachedFetch (options, cb) {
            return ParentKlass.prototype.callApiClient.call(that, method, options, function recordHashKey (err) {
              // If there was an error, callback immediately
              if (err) {
                return cb.apply(this, arguments);
              }

              // Otherwise, add the hash to our hash store
              var callbackContext = this;
              var args = [].slice.call(arguments);
              that.redis.sadd(that._getCacheHashKey(), optionsHash, function recordHashKeySave (err) {
                // If there was an error, callback with it
                if (err) {
                  return cb(err);
                }

                // Otherwise, callback with the original data
                return cb.apply(callbackContext, args);
              });
            });
          }
        }, callback);
...
exports.mixinModel = function (ModelKlass) {
  ...
  return ModelKlass.extend(_.extend(MixinBase, {
    ...
    _getCacheHashKey: MixinBase._getModelHashKey
  }));
};
...
exports.mixinCollection = function (CollectionKlass) {
  ...
  return CollectionKlass.extend(_.extend(MixinBase, {
    ...
    _getCacheHashKey: MixinBase._getCollectionHashKey,
